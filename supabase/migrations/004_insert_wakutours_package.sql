...
INSERT INTO public.tourism_providers (
  ...
  '["Paquetes turísticos", "Tours", "Hospedaje", "Excursiones"]'::jsonb,
  '["Los Roques", "Margarita", "Morrocoy"]'::jsonb,
  4.8,
  NOW()
)
...
SELECT
  (SELECT id FROM public.tourism_providers WHERE slug = 'wakutours'),
  (SELECT id FROM public.destinations WHERE slug = 'los-roques'),
  (SELECT id FROM public.catalog_services WHERE slug = 'packages'),
