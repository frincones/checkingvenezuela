  ...
  for (const config of chain) {
    try {
      ...
      const result = streamText({
        ...
        temperature: 0.4,
        // Forzar tool SOLO en el primer paso; pasos siguientes son libres
        // (sino el modelo entra en loop y/o falla con "did not call a tool")
        prepareStep: ({ stepNumber }) => {
          if (forceTool && tools[forceTool] && stepNumber === 0) {
