...
function fastIntentByKeywords(message) {
  ...
  // 'destino' captura "destino/destinos" (cliente preguntando por catálogo).
  if (
    /\b(cotiz|reserv|compr|precio|paqu|vuelo|vuelos|hotel|hoteles|tour|escapada|disponibil|destino|book|quote|buy|price|destination)/.test(
      t
    )
  ) {
    return "booking";
  ...
  // Preguntas exploratorias del catálogo: "qué tienen", "qué ofrecen",
  // "qué hay disponible", "qué opciones hay", "qué pueden ofrecerme",
  // "what do you offer". El cliente quiere ver el catálogo → booking.
  if (
    /\b(qu[eé]\s+(tienen|ofrecen|ofertan|hay|opciones|lugares|destinos)|what\s+(do\s+you\s+(have|offer)|destinations))/i.test(
      t
    )
  ) {
    return "booking";
  ...
  // Destino solo (mensaje corto, sin verbo): el cliente dice "margarita" /
  // "los roques" / "canaima" tras un saludo. Tratamos como booking — el
  // objetivo del agente es mostrar paquetes, no solo info del destino.
  if (t.split(/\s+/).filter(Boolean).length <= 4 && destinations.test(t)) {
    return "booking";
  }
