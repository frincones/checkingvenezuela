...
export const searchDestinationsTool = tool({
  ...
  inputSchema: z
    ...
    .passthrough(),
  execute: withToolTimeout(async ({ query, country, limit = 5 }) => {
    try {
      const sb = admin();
      ...
      let q = sb
        ...
        // Sin este filtro el chatbot ofrecía los destinos desactivados
        // (los hoteles internacionales y los archivados por la dedup ES/EN).
        .eq("is_active", true)
