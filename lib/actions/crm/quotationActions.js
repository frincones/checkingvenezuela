...
/**
 * Server Actions para Cotizaciones - VENEZUELA VOYAGES CRM
 *
...
export async function updateQuotationItemsAction(quotationId, items) {
  try {
    ...
    // I2: con un cobro vivo, editar los items haría que la factura de PayPal
    // y el PDF ya enviado al cliente dejaran de cuadrar.
    const lock = await getActivePaymentLock(quotationId);
    if (lock.locked) {
